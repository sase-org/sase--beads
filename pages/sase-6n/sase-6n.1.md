# Bead: sase-6n.1 — Clan wire model and runtime aggregation in sase-core

[Bead Pages](../README.md) / [sase-6n](README.md) / sase-6n.1

**Status:** ✓ closed · **Resolution:** (unrecorded) · **Type:** phase
**Owner:** `bryanbugyi34@gmail.com` · **Assignee:** `sase-6n.1`
**Created:** 2026-07-17 21:34:45 UTC
**Plan:** [202607/agent\_clans\_families\_tribes.md](https://github.com/sase-org/sase--plans/blob/main/202607/agent_clans_families_tribes.md)

## Description

'Phase core-runtime' section: add the agent_clan wire field with legacy mapping and implement interval-union clan runtime (excluding human-wait) in sase_core plus Python facade mirrors.

## Notes

COMMIT: cfc517ace

## Dependencies

- **Blocks:** [sase-6n.2](sase-6n.2.md) ✓
- **Blocks:** [sase-6n.6](sase-6n.6.md) ✓

## Agents

| Agent | Bead | Commits |
|---|---|---:|
| [bbugyi200.athena.sase-6n.1](https://github.com/sase-org/sase--agents/blob/main/agents/bbugyi200.athena.sase-6n.1/README.md) | [sase-6n.1](sase-6n.1.md) | 1 |

## Commits

| Commit | Subject | Bead | Committed (UTC) |
|---|---|---|---|
| [`35c44d8`](https://github.com/sase-org/sase/commit/35c44d8221717b7c70c9e1552402f2d51901f33c) | feat(runtime): expose clan wall-clock aggregation (sase-6n.1) | [sase-6n.1](sase-6n.1.md) | 2026-07-17 22:02:23 |
