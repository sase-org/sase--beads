# Bead: sase-xz.3 — Responsive syntax composition and styled search

[Bead Pages](../README.md) / [sase-xz](README.md) / sase-xz.3

**Status:** ◐ in_progress · **Type:** ↳ phase
**Owner:** `bryanbugyi34@gmail.com` · **Created by:** [bbugyi200.athena.03g](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.03g.md) · **Assignee:** `sase-xz.3` · **Size:** medium
**Created:** 2026-09-07 10:51:42 EDT
**Plan:** [202609/pager\_filetype\_syntax.md](https://github.com/sase-org/sase--plans/blob/main/202609/pager_filetype_syntax.md)

## Description

reading_surface: implement optional section metadata, pump-free preparation, bounded caches, source-preserving label composition, styled search, theme invalidation, and the language hint. Exercise explicit test inputs while leaving existing producers inert.

## Dependencies

- **Depends on:** [sase-xz.1](sase-xz.1.md) ✓ · ⧖ 2026-09-07
- **Depends on:** [sase-xz.2](sase-xz.2.md) ✓ · ⧖ 2026-09-07
- **Blocks:** [sase-xz.4](sase-xz.4.md) ◐ · ⧖ 2026-09-07

## Agents

| Agent | Bead | Commits |
|---|---|---:|
| [bbugyi200.athena.sase-xz.3](https://github.com/sase-org/sase--agents/blob/main/agents/bbugyi200.athena.sase-xz.3/README.md) | [sase-xz.3](sase-xz.3.md) | 1 |

## Commits

| Repo | Commit | Subject | Bead | Committed |
|---|---|---|---|---|
| sase | [`a9f95ca`](https://github.com/sase-org/sase/commit/a9f95ca5e64510df9f6161ee259d4d99d042537f) | feat(pager): thread source-language syntax hints through screen and layout | [sase-xz.3](sase-xz.3.md) | 2026-09-07 14:10:25 EDT |
