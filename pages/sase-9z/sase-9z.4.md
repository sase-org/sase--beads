# Bead: sase-9z.4 — Show the logical reference and its resolved path

[Bead Pages](../README.md) / [sase-9z](README.md) / sase-9z.4

**Status:** ✓ closed · **Resolution:** (unrecorded) · **Type:** phase
**Owner:** `bryanbugyi34@gmail.com` · **Assignee:** `sase-9z.4` · **Size:** small
**Created:** 2026-07-27 12:39:35 UTC · **Closed:** 2026-07-27 15:51:46 UTC
**Plan:** [202607/durable\_plan\_refs.md](https://github.com/sase-org/sase--plans/blob/main/202607/durable_plan_refs.md)

## Description

display: render the stable reference together with where it currently resolves, and say so plainly when it resolves nowhere, in `sase bead show` and the ACE Plans tab.

## Dependencies

- **Depends on:** [sase-9z.1](sase-9z.1.md) ✓
- **Depends on:** [sase-9z.2](sase-9z.2.md) ✓

## Agents

| Agent | Bead | Commits |
|---|---|---:|
| [bbugyi200.athena.sase-9z.4](https://github.com/sase-org/sase--agents/blob/main/agents/bbugyi200.athena.sase-9z.4/README.md) | [sase-9z.4](sase-9z.4.md) | 2 |

## Commits

| Commit | Subject | Bead | Committed (UTC) |
|---|---|---|---|
| [`sase-core@b230538`](https://github.com/sase-org/sase-core/commit/b2305382a657750efaf2fe308181f2325eb9c729) | feat(bead): show plan references alongside where they resolve (sase-9z.4) | [sase-9z.4](sase-9z.4.md) | 2026-07-27 15:52:40 |
| [`881636a`](https://github.com/sase-org/sase/commit/881636a745de9f88d43cbfcec868f6a537e9f0a2) | feat(sdd): surface plan references and where they resolve (sase-9z.4) | [sase-9z.4](sase-9z.4.md) | 2026-07-27 16:03:49 |
